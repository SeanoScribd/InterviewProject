...
import com.sean.oxford.scribdproject.repository.hasConnectivity
...
abstract class BaseApiHandler<ViewState, NetworkObj> constructor(
        ...
        return withContext(IO) {
            try {
                ...
                when (throwable) {
                    ...
                    is IOException -> {
                        ApiResult.GenericError(
                            69, getString(R.string.dialog_message_error_default)
                        )
                    }
                    is HttpException -> {
    ...
    abstract suspend fun createSuccessViewState(data: NetworkObj, stateEvent: StateEvent?): DataState<ViewState>
